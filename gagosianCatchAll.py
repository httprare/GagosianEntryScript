...
catchall = "yourdomain.whatever"
def main():
    entries = int(input("How many Times would you like to enter?: "))
    Size = ['XS','S','M','L','XL']
    i = 0
    while i < entries:
        proxy_list = open(file, 'r')
        proxysplit = proxy_list.read().splitlines()
        proxypuller = random.choice(proxysplit)
        proxies = {
        'https': proxypuller
        }
        print('Using Proxy ' + proxypuller + ' to fetch Gagosian...')
        tSize = random.choice(Size)
        print(tSize)
        RandomEntryEmail = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(8)])
        print("Using " + RandomEntryEmail  + "@" + catchall + "to enter the Raffle...")
        print("Entry #" + str(i) +  " of " + str(entries) + ":")
        payload = "name=FirstName%20LastName&email=" + RandomEntryEmail + "@" + catchall + "&size-value=" + tSize + "&year=2018&lottery=yes"
        headers = {
        ...
        try:
            ...
            if "Your submission number is" in response.text:
                print("Successful Entry with Email: "+ RandomEntryEmail + "@" + catchall  + " in size " + tSize +". Entry Number: " + EntryNum)
                i = i + 1
            if str(soup.find("div", attrs={'role': 'alert'})) in response.text:
                print('Duplicate Entry. Tryagain with a new set of info...')
            time.sleep(1)
        except requests.exceptions.ProxyError:
            print('This proxy didn\'t work the first time. Trying a new one...')
